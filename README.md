...
#### 실시간/스토리지 (관리자 페이지에서 설정)

Realtime과 Storage 설정은 관리자 페이지 → 전역 설정에서 관리합니다. 환경 변수는 개발 편의용 fallback으로만 사용됩니다.
...
컨테이너 이미지가 GitHub Container Registry에 자동 빌드됩니다 (amd64/arm64).
...
# MAXMIND_LICENSE_KEY=your_key bash deploy/setup.sh
...
`deploy/openchamchijs.container`를 `~/.config/containers/systemd/`에 복사하고 환경 변수 파일을 준비합니다.

```bash
cp deploy/openchamchijs.container ~/.config/containers/systemd/
mkdir -p ~/.config/openchamchijs
# .env 파일 생성
systemctl --user daemon-reload
systemctl --user start openchamchijs
```
