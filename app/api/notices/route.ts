...
// GET /api/notices - 글로벌 공지사항 목록 조회
export async function GET(request: NextRequest) {
  const { searchParams } = new URL(request.url);
  const { page, limit, search } = parsePaginationQuery(searchParams, { limit: 20 });

  try {
    const result = await noticeService.findGlobal({ page, limit, search });
...
export async function POST(request: NextRequest) {
  ...
  try {
    const notice = await noticeService.createGlobal(session.user.id, parsed.data);
