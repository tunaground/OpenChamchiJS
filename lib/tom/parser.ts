...
export const SELF_CLOSING_TAGS = ["youtube", "hr", "dice"] as const;
...
export function parse(input: string): TomRoot {
      ...
      if (token.type === "text") {
        ...
        if (isValidTag(normalized)) {
          ...
          stack.push({
            ...
            isSelfClosing: isSelfClosingTag(normalized),
          });
  ...
  while (stack.length > 1) {
    ...
    if (frame.context === "nested") {
      ...
      const nestedText = "(" + stringifyNodes(frame.nodes);
      ...
      if (isTomElement(lastElement) && lastElement.name === frame.tagName) {
        ...
        const attrText = stringifyNodes(frame.nodes);
        if (attrText) {
          text += " " + attrText;
        }
...
function stringifyNodes(nodes: TomNode[]): string {
  let result = "";
  for (const node of nodes) {
    if (isTomText(node)) {
      result += node.value;
    } else if (isTomNested(node)) {
      result += "(" + stringifyNodes(node.children) + ")";
    } else if (isTomElement(node)) {
      const attrs = stringifyNodes(node.attributes);
      const children = stringifyNodes(node.children);
...
// Stringify: Convert AST back to TOM string
export function stringify(root: TomRoot): string {
